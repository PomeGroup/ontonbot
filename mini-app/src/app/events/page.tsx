...
const EventsAdminPage = () => {
    ...
    const validatedData = trpc.users.validateUserInitData.useQuery(initData || '');
    ...
    return (
        <div>
            <Link href={`/events/create`} className="w-full" onClick={() => WebApp?.HapticFeedback.impactOccurred('medium')}>
            ...
            {eventsData?.data?.map((event) => (
                <div key={`event-${event.event_id}-link`} className="my-4">
                    <Card className="flex flex-col w-full p-0 my-0">
                        ...
                        <div className="w-full flex flex-col gap-2">
                            <div className="text-primary p-4 pb-1">
                                ...
                                <Labels.CampaignDescription className="text-secondary" description={event.subtitle!} />
                            </div>
                            ...
                            <div className="flex flex-col gap-1 p-4 pt-1">
                                ...
                                <div className="flex flex-row whitespace-nowrap gap-3 text-primary text-sm items-center">
                                    <TimeRow start_date={event.start_date!} end_date={event.end_date} timeZone={event.timezone!} />
                                </div>
                            </div>
                        </div>
                    </Card>
                    ...
                    <div className="flex gap-2 mt-2">
                        <Link className="flex-1" href={`events/${event.event_uuid}/edit`} onClick={() => WebApp?.HapticFeedback.impactOccurred('medium')}>
...
const TimeRow = ({
    ...
    const formatTime = (time: { hours: string; minutes: string }) => `${time.hours}:${time.minutes}`;

    // Helper to format date
    const formatDate = (date: { day: string; month: string }) => `${date.day}/${date.month}`;
    ...
    if (end_date) {
        ...
        if (endDate && startDate && formatDate(endDate) === formatDate(startDate)) {
            displayString += `- ${formatTime(endTime!)} ${formatDate(startDate)} ${timeZone}`;
        }
        // If there's an end date and it differs from the start date
        else if (endDate && startDate) {
            displayString += `${formatDate(startDate)} - ${formatTime(endTime!)} ${formatDate(endDate)} ${timeZone}`;
        }
    } else {
