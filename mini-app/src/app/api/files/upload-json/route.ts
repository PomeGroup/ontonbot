...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (formFile.size > 200 * 1024) {
      return NextResponse.json({ message: "JSON file size exceeds the 200 KB limit." }, { status: 400 });
    }
    ...
    return NextResponse.json({ message: "An error occurred during JSON upload." }, { status: 500 });
  }
}
