...
const ImageUpload = ({ isError, clearError }: { isError: boolean; clearError: () => void }) => {
...
const firstStepDataSchema = z.object({
  title: z.string().min(2, { message: "Title must be at least 2 characters" }).max(40, { message: "Title must be less than 40 characters" }),
  subtitle: z.string().min(2, { message: "Subtitle must be at least 2 characters" }).max(100),
 // description: z.string().min(1, { message: "Description must be at least 1 character" }),
  image_url: z.string({ required_error: "Please select an image" }).url({ message: "Please select an image" }),
  ...
  return (
      <form ref={formRef} onSubmit={handleSubmit}>
        <StepLayout>
          ...
          {/* Image Upload */}
          <ImageUpload isError={Boolean(errors?.image_url)} clearError={clearImageError} />
