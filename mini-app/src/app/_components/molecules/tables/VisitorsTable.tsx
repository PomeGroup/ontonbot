...
const VisitorsTable: FC<VisitorsTableProps> = ({ event_uuid }) => {
    ...
    const flatData = data?.pages?.flatMap((page) => page?.visitorsWithDynamicFields) ?? [];

    return (
        <div className="mt-4 overflow-x-auto">
            <div className="">
                <div className="text-sm sm:text-md md:text-lg gap-4 p-4" style={{ display: 'grid', gridTemplateColumns: '2fr 2fr minmax(auto, max-content)' }}>
                    ...
                    {flatData.map((visitor, index) => {
                        ...
                        return (
                            <Fragment key={index} >
                                <div ref={index === flatData.length - 1 ? ref : null} className="truncate">{`${visitor?.first_name} ${visitor?.last_name}`}</div>
                                <div
                                ...
                                >
                                    {visitor?.wallet_address ? (
                                        <>
                                            Open
                                            <Wallet2 className='ml-1' width={12} height={12} />
                                        </>
                                    ) : (
                                        <>
                                            No Wallet
