...
export const usersRouter = router({
    ...
    haveAccessToEventAdministration: publicProcedure.input(z.string()).query(async (opts) => {
        if (!opts.input) {
            return undefined
        }
        ...
        try {
            ...
            const userRole = await db.select({ role: users.role }).from(users).where(eq(users.user_id, data.initDataJson.user.id)).execute()

            if (!userRole || (userRole[0].role !== 'admin' && userRole[0].role !== 'organizer')) {
                return false
            }
    ...
    // private
    addUser: publicProcedure.input(z.object({ initData: z.string() })).mutation(async (opts) => {
    ...
    // private
    getWallet: publicProcedure.input(
        z.object({ initData: z.string().optional() })
    ).query(async (opts) => {
        if (!opts.input.initData) {
            return
        }
