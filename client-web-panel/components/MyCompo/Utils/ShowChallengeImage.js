...
const ShowChallengeImage = ({ challenge, width = 40, height = 40, className = "borRadius100", enableDialog = false }) => {
    ...
    return (
        <>
            {challenge?.poster ? (
                <>
                    <Image
                        src={srcImg(challenge.poster)}
                        alt={challenge?.title || 'User'}
                        width={width}
                        height={height}
                        className={className}
...
export default ShowChallengeImage;
