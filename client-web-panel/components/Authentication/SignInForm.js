...
import { useSendOtpCodeMutation, useLoginWithOtpMutation } from '/redux/slices/authApiSlice'; // Adjust the path as necessary
...
const SignInForm = () => {
    ...
    const [sendOtpCode, { isLoading: isRequestLoading }] = useSendOtpCodeMutation();
    const [loginWithOtp, { isLoading: isVerifyLoading }] = useLoginWithOtpMutation();
    ...
    const handleVerificationSubmit = async (event) => {
        ...
        try {
            ...
            const response = await loginWithOtp({ organizerId, userId, loginCode: verificationCode }).unwrap();
            if (response.success) {
    ...
    return (
      <div className="authenticationBox">
          <Box component="main" sx={{ maxWidth: "510px", ml: "auto", mr: "auto", padding: "50px 0 100px" }}>
              <Grid item xs={12} md={12} lg={12} xl={12}>
                  <Box>
                      <Box className={styles.or} sx={{width:"200px",mr:"auto",ml:"auto"}}>
                          <NextLink href='/'>
                      ...
                      {!showVerificationField ? (
                        ...
                        <Box component="form" noValidate onSubmit={handleOtpRequestSubmit}>
                            <Box sx={{ background: "#fff", padding: "30px 20px", borderRadius: "10px", mb: "20px" }} className="bg-black">
                            ...
                            <Button type="submit" fullWidth variant="contained" sx={{ mt: 2, textTransform: "capitalize", borderRadius: "8px", fontWeight: "500", fontSize: "16px", padding: "12px 10px", color: "#fff !important" }}>
                                {t('common.get_code')}
                            </Button>
                        </Box>
                      ) : (
                        // OTP Verification Form
                        <Box component="form" noValidate onSubmit={handleVerificationSubmit}>
                            <Box sx={{ background: "#fff", padding: "30px 20px", borderRadius: "10px", mb: "20px" }} className="bg-black">
                                <Grid container alignItems="center" spacing={2}>
                                    <Grid item xs={12}>
                                        <Typography component="label" sx={{ fontWeight: "500", fontSize: "14px", mb: "10px", display: "block" }}>
                                        ...
                                        <Typography variant="body2" sx={{ mt: 2 }}>
                                            {t('common.code_sent_to')} {organizerId}. <NextLink href="#" onClick={handleEditUserIds}>{t('common.change_ids')}</NextLink>
                                        </Typography>
                                    </Grid>
                                    <Grid item xs={12}>
