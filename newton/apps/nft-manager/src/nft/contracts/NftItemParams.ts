...
import { ParamsValue } from "../../helper";
...
export function storeNftItemParams(src: NftItemParams) {
  return (builder: Builder) => {
    ...
    if (typeof src.individualContent === "string") {
      builder.storeRef(
        beginCell().storeStringTail(src.individualContent).endCell(),
      );
    } else {
...
export function loadNftItemParams(
  slice: Slice,
): NftItemParams & { individualContent: Cell } {
  return {
