...
import { LoadParams, StoreParams } from "../../helper";
...
export function loadNftBatchMintMessage<T>(
  slice: Slice,
  loadParams: LoadParams<T>,
): NftBatchMintMessage<T> {
...
export function storeNftBatchMintItem<T>(
  ...
  return (builder: Builder) => {
    builder.storeCoins(request.value ?? toNano("0.05"));
    builder.storeRef(beginCell().store(storeParams(request.params)).endCell());
  };
}

export function loadNftBatchMintItem<T>(
  slice: Slice,
  loadParams: LoadParams<T>,
): NftMintItem<T> {
